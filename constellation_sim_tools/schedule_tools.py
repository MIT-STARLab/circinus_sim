...
class ExecutableActivity:
    """
    this is an object that keeps track of an activity window, and the route containers (with their underlying data
    routes) whose executions are the reason why the activity window is being performed. this helps us keep track of
    why a window is being performed and where data goes to and arrives from
    """
    ...
    def __init__(self,wind,rt_conts,dv_used,injected=False):
        ...
        self.wind =  wind
        # rt_conts are the
        self.rt_conts = rt_conts   # route containers for the window
        # dv_used is the
        self.dv_used = dv_used      # amount of data volume used for this window within the route
                                    # in which the wind was found

        # Used to indicate if the activity is "injected" or not. "injected" means that
        # it's a spontaneous/unplanned-for activity
...
def synthesize_executable_acts(rt_conts,filter_start_dt=None,filter_end_dt=None,filter_opt='partially_within',
                               sat_indx=None,gs_indx=None,act_timing_helper=None):
    """
    go through all of the route containers and synthesize a list of unique windows to
    execute with the correct time and the data volume utilization
    """


    # First we need to find all of the executable versions of each activity window contained anywhere
    # in the data routes in the route containers.
    # note!  there may be duplicate copies of wind residing within the executable Windows objects.
    # in general this is okay though because we use their window ID hash for lookup
    exec_acts_by_wind = {}
    ...
    #  then we need to synthesize these possibly disagreeing executable versions of each activity window into
    #  a single executable version; that is, we need to figure out the time utilization and DV utilization from
    #  every Sim route container and take the maximum over those. we take the sum because every route only accounts
    #  for how much of the activity window IT needs to use. if we take the sum over all routes, then we know how much
    #  of the window we actually need to use across all routes
    executable_acts_sythesized = []
    for act,exec_acts in exec_acts_by_wind.items():
        # exec_act is of type ExecutableActivity
        
        #  these are the amounts of data volume used for every route passing through this window ( well, every
        #  route within our filter)
        ...
        #  do some quick sanity checks. every window should be the same, and the sum of the time and data volume
        #  utilizations should be less than or equal to full utilization (100%)
        #  note that this window equality check checks the hash of the window (window ID). they don't have to be
        #  exactly the same python runtime object, just the same activity in the simulation
...
def check_temporal_overlap(window_start_dt,window_end_dt,filter_start_dt=None,filter_end_dt=None,filter_opt='partially_within'):
    ...
    Filters a monolithic window of interest based upon a filter window. looks at whether or not the two windows
     overlap each other
