...
class SimDataContainer:
    ...
    # note this route is simple:  there are no forks in the route; there is a simple linear path from an observation
    # to a downlink through which data flows. all windows must be in temporal order.
    ...
    def __init__(self,agent_id,sat_indx,dc_indx,route,dv=0):
        ...
        if type(route) == ObsWindow:
            obs = route
            route = [obs]
            # this stores the data route taken by the observation data in this container. Note that it is a simple
            # data route, NOT a data multi route
        ...
        # Contains all of the route containers that specified the plans for routing this data. this history is
        # added to every time data is moved between agents.
        # Note that when planning info is updated, this planned route history should be updated as well, if the
        # global planner is intended to update existing data conts
    ...
    def injected(self):
        ...
        return self._executed_dr.get_obs().injected

    @property
    def latest_planned_rt_cont(self):
        """
        get the latest planned route for this data container. This can be trusted as the specification of where
        the data is planned to be sent, at any given time
        """

        # Note that it is an error for the planned route history to be empty -  it should always be populated by
        # at least one entry after a new Sim data route is created
        latest_planned_rt_cont = self._planned_rt_hist[-1]
            
        #  if there is a (most recent) planned route for this data container, then the executed data route should
        #  be contained within that planned route. this is basically saying that the plans for the data container
        #  must be up-to-date with its actual execution
        if latest_planned_rt_cont:
            if not latest_planned_rt_cont.contains_route(self._executed_dr):
                raise RuntimeWarning(' executed data route not contained within the latest planned route for data '
                                     'container. Executed route: %s, latest planned route: %s'
                                     % (self._executed_dr,latest_planned_rt_cont))
    ...
    def add_to_plan_hist(self,rt_cont):
        ...
        if len(self._planned_rt_hist) > 0:
            #  sanity check: None is a valid option for the first step of the route history, If observation data was
            #  collected without a route in mind, but none is not a valid option after that point,  because cross-links
            #  and down links should always have a route in mind when sending data
            assert(rt_cont is not None)

            #  because this function might be called many times for a single transmission activity, we want to check
            #  and make sure that we haven't already added this route container at the end of the history
...
class SimRouteContainer:
    """
    This object wraps the data routes produced in both the global and local planners, for use in simulation. it can
    essentially be thought of as a data route, in the context of the constellation simulation.

    This contains lower level data route objects, for use in the constellation simulation. It effectively allows the
    simulation to easily vary the amount of data volume, time scheduled for a data route, in order to flexibly replan in realtime.

    The use of this container is an artifact of the choice to model data routes as a series of activity windows of
    fixed start/end times. If we wish to change any of those start/end times, that would be reflected as a change in
    the underlying window object, but not in the data route object itself, which is not great. So we'd have to create
    a new data route object - which is not great for rescheduling because its harder to track the data route object as
    it gets passed around the constellation. By wrapping the data route in a container that can change its state, we
    have both timing/dv flexibility and persistent object indexing"""

    #  note that this container currently can only hold data multi-routes. 
    # note that i put the mechanisms in place here to have multiple DMRs in a single sim route container, but I don't
    # use that capability currently. Mildly regretting doing so...
    ...
    def __init__(self,ro_ID,dmrs,dv_utilization_by_dmr_id,creation_dt,update_dt,creator_agent_ID):
        ...
        for dmr in dmrs_by_id.values():
            ...
            #     raise RuntimeWarning('For current version of global planner, expect time and data volume
            #     utilization for a data route to be the same. DMR: %s'%(dmr))
        ...
        # this is the "time utilization" for the data route (DMR), which is a number from 0 to 1.0 by which the
        # duration for every window in the route should be multiplied to determine how long the window will actually be
        # executed in the real, final schedule.
        # self.t_utilization_by_dr_id = t_utilization_by_dr_id
        # this is the "data volume utilization" for the data route (DMR), which is a number from 0 to 1.0 by which the
        # scheduled data volume for every window in the route should be multiplied to determine how much data volume will
        # actually be throughput on this dr in the real, final schedule
    ...
    def __copy__(self):
        # want to make sure that the copy made of this route container copies the underlying data route objects,
        # so that route containers don't end up sharing these.  for this reason, this copy function should
        # be used sparingly
        #  note the assumption here that ideas will never be changed at a low level, so copying the reference is fine
        #  the nones in the instantiation will be taken care of in the following lines
        newone = type(self)(ro_ID=self.ID, dmrs=None, dv_utilization_by_dmr_id=None, creation_dt=self.creation_dt,
                            update_dt=self.update_dt, creator_agent_ID=self.creator_agent_ID)
    ...
    def get_display_string(self):
        return 'utilization_by_dmr: %s'%({'DMR %s - '%(dr_id) +self.dmrs_by_id[dr_id].get_display_string():util
                                          for dr_id,util in self.dv_utilization_by_dmr_id.items()})
    ...
    def get_next_planned_wind(self,rt):
        ...
        # if we're checking for a next planned route, should be sure rt is contained in self first before calling
        # this func. Raise warning here to warn about bad situations otherwise.
    ...
    def updated_check(self,dmr,dmr_dv_util,dv_utilization_epsilon=None):
        """
        Returns true if self is not updated; that is, utilization for contained datamultiroute has not changed
        significantly
        """
    ...
    def get_winds_executable(self,filter_start_dt=None,filter_end_dt=None,filter_opt='partially_within',sat_indx=None,gs_indx=None):
        ...
        #  note: using a list here instead of a set because in general the list should be small and
        #  okay for membership checking
        winds_executable = []
        ...
        for dmr in self.dmrs_by_id.values():
            ...
            for wind in winds:
                ...
                #  create a new executable window entry, which specifies both the window and the amount of data
                #  volume used from it for this route
    ...
    def data_vol_for_wind(self,wind):
        ...
        if wind_sum == 0 and self.data_vol > self.dv_epsilon:
            raise KeyError('Found zero data volume for window, which assumedly means it is not '
                           'in the route. self: %s, wind: %s'%(self,wind))
    ...
    def find_matching_data_conts(self,data_conts,match_method='planned'):
        ...
        This method is essentially the linchpin in matching schedule intent ( represented by the sim route container)
        to execution ( represented by the data container). A data container is said to match a route container
        in two possible ways:

        1. "planned": if its latest planned data route matches at least one of the data routes underlying the
           Sim route container
        2. "executed": if its executed route matches at least one of the data routes underlying the Sim route container 
        
        This route matching is on a window by window basis. that is, the data container route has the same observation
        and same cross-links as a data route within the Sim route container, up to and including the latest cross-link
        in the data container. This could also include the final downlink, but it's likely not useful to do this matching
        process after the downlink has occurred. Note that no data volume constraints are set on this match -  only the
        ...
        # note that the success of this matching process depends on having up-to-date plans ( route containers)
        # and marking those updates for each data container. If plans have not been updated, then you might end
        # up in a situation where there is a route container that expresses routing plans, but no data container
        # can be found to service those plans
...
class ExecutableDataContainer:
    """
    This object holds a data container alongside the remaining data volume allowed to be routed for it in a given
    context
    """
